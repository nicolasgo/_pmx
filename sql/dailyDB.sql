...
create table zzuserlog ENGINE=MyISAM DATA DIRECTORY='/var/mysql-data/smp/' INDEX DIRECTORY='/var/mysql-data/smp/' select * from smp_usage_log as l where rel_operator_id=10 and action <>15 and time >=@_5days;

alter table zzuserlog add COLUMN user_id bigint(20) unsigned;
create index user_id on zzuserlog(user_id);


/***/
delete from zzuserlog where time < @_10days;
delete from zzuserlog where time > @_yesterday and time < @_today;

insert into zzuserlog(usage_log_id,msisdn,rel_operator_id,time,action,device,miscid1,miscid2,miscstr1,miscstr2,miscstr3) select * from smp_usage_log as l where rel_operator_id=10 and action <>15 and time >= @_yesterday and time < @_today;
...
create table zzuser ENGINE=MyISAM DATA DIRECTORY='/var/mysql-data/smp/' INDEX DIRECTORY='/var/mysql-data/smp/' select user_id,msisdn,rel_operator_id as oid,joined,last_seen,disk_usage,state from smp_user where rel_operator_id=10 and state=1;
...
update zzuserlog as l inner join zzuser as u using(msisdn) SET l.user_id=u.user_id where l.time > @_10days;
#update zzuserlog as l inner join zzuser as u using(msisdn) SET l.user_id=u.user_id where l.time > @_yesterday;
#update zzuserlog as l inner join zzuser as u using(msisdn) SET l.user_id=u.user_id ;
/****/
...
update zzuser as u set u.area=substring(u.msisdn,1,5);


#
drop table if exists zzfollowing;
create table zzfollowing ENGINE=MyISAM DATA DIRECTORY='/var/mysql-data/smp/' INDEX DIRECTORY='/var/mysql-data/smp/' select rel_followee_id as user_id,rel_user_id as following from smp_following where rel_followee_id in (select user_id from zzuser);

#
drop table if exists zzdevices;
create table zzdevices ENGINE=MyISAM DATA DIRECTORY='/var/mysql-data/smp/' INDEX DIRECTORY='/var/mysql-data/smp/' (select user_id,date(time) as date,miscstr3,device from zzuserlog group by msisdn,date,device);

#
drop table if exists zzfriends;
#create table zzfriends ENGINE=MyISAM DATA DIRECTORY='/var/mysql-data/smp/' INDEX DIRECTORY='/var/mysql-data/smp/' select rel_user_id as user_id,rel_friend_id,status from smp_contact_friend where rel_user_id in (select user_id from zzuser where last_seen >@_60days);
